...
Write-Host "Microsoft Visual J# .NET 2.0"
Start-Process "C:\navdvd\Prerequisite Components\Microsoft Visual J# .NET 2.0\vjredist20.exe" -ArgumentList "/q:a /c: install /q" -Wait

Write-Host "Installing VC Redist"
#Start-Process "C:\navdvd\Prerequisite Components\Microsoft Visual C++ 2008\vcredist_x64.exe" -ArgumentList "/passive /norestart" -Wait
Start-Process "C:\navdvd\Prerequisite Components\Microsoft Visual C++ 2008\vcredist_x86.exe" -ArgumentList "/qb" -Wait
...
Write-Host "Microsoft Core XML Services (MSXML) 6.0 Service Pack 1"
start-process "$NavDvdPath\Prerequisite Components\Microsoft Core XML Services (MSXML) 6.0 Service Pack 1\msxml6_x64.msi" -ArgumentList "/quiet /qn /passive" -Wait
...
Write-Host "Copying ClientUserSettings"
Copy-Item (Join-Path (Get-ChildItem -Path "$NavDvdPath\RoleTailoredClient\CommonAppData\Microsoft\" -Directory | Select-Object -Last 1).FullName "ClientUserSettings.config") $runPath
